      ...
      test('Test that pulling a small model works', () async {
        try {
          ...
          await for (final progress in repository.pullModel(testModel)) {
            ...
            if (progress.status.contains('downloading') &&
                progress.progress > 0.01) {
              print(
                'Model pull started successfully, stopping test to avoid full download',
              );
              break;
            }
          }
          ...
          expect(
            hasStarted,
            isTrue,
            reason: 'Pull operation should have started',
          );
          print(
            'Pull test completed. Started: $hasStarted, Complete: $isComplete',
          );
        } catch (e) {
          print('Error pulling model: $e');
          print(
            'This might be expected if the model is not available or network issues occur',
          );
