...
class OllamaRepository {
  OllamaRepository({this.baseUrl = "http://localhost:11434", HttpClient? httpClient})
  ...
  Stream<OllamaPullProgress> pullModel(String modelName) async* {
    final request = await httpClient.openUrl('POST', Uri.parse('$baseUrl/api/pull'));
  ...
  Future<OllamaVersion> version() async {
    final response = await _sendRequest('GET', Uri.parse('$baseUrl/api/version'));
    final body = await response.transform(utf8.decoder).join();
    final json = jsonDecode(body);
